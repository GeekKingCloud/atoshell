...
- Fixed large cross-file ticket state moves by keeping full ticket payloads out of process arguments, avoiding Windows command-line limits for long descriptions and comment histories.
- Polished state handling, CLI documentation, and local verification guidance so contributor checks stay focused on affected behavior.
- Tightened terminal alignment contracts in `STYLE.md` and pinned the complete measured runtime/README menu grid.
- Strengthened generated config alignment checks so status comments remain two spaces after the widest assignment.

### Tests

- Added regression coverage for large ticket moves, menu/README alignment parity, and config template comment-column alignment.
